...
const AdmissionDish = () => {
  ...
  useEffect(() => {
    http.get<IDish[]>("pratos/").then((response) => setDishes(response.data));
  }, []);

  const exclude = (dishToBeDeleted: IDish) => {
    http.delete(`pratos/${dishToBeDeleted.id}/`).then(() => {
      const listDishes = dishes.filter(
        (dish) => dish.id !== dishToBeDeleted.id
      );
  ...
  return (
    <>
      <Grid container>
        ...
        <Grid item>
          <Link variant="button" component={RouterLink} to="/admin/pratos/novo">
            Novo
          </Link>
        </Grid>
      </Grid>
          ...
          <TableBody>
            {dishes?.map((dish) => (
              <TableRow key={dish.id}>
                ...
                <TableCell>
                  <a href={dish.imagem} target="_blank" rel="noreferrer">
                    Ver Imagem
                  </a>
                </TableCell>
                <TableCell>
